...
export default function BookingModal({ whatsappNumber, whatsappNumber2, googleCalendarEnabled = false, clinicName = "Venegas Costa Rica Tours", clinicAddress = "" }: Props) {
  ...
  async function go() {
    ...
    if (method === "whatsapp") { openWhatsApp({ number: whatsappNumber, name: f.name, service: f.service, date: f.date, time: f.time }); if (whatsappNumber2) { setTimeout(() => openWhatsApp({ number: whatsappNumber2, name: f.name, service: f.service, date: f.date, time: f.time }), 1000); } setRes({ ok: true, msg: "WhatsApp opened. We'll respond shortly!" }); setBusy(false); return; }
