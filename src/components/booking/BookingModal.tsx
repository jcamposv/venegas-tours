...
const SERVICES = ["Arenal Volcano Tour", "Miravalles Volcano Tour", "Rio Celeste Tour", "Rainforest Hanging Bridges", "Sunset Sailing", "Dinner Cruise", "Catamaran Full Day", "Beach Tour (Conchal/Flamingo/Tamarindo)", "Custom Itinerary"];
const TIMES = ["07:00","08:00","09:00","10:00","11:00","13:00","14:00","15:00","16:00"];

export default function BookingModal({ whatsappNumber, googleCalendarEnabled = false, clinicName = "Venegas Costa Rica Tours", clinicAddress = "" }: Props) {
  ...
  const inp = "w-full rounded-xl border border-charcoal/10 bg-white px-4 py-3 text-sm outline-none transition-colors focus:border-[#1a4d2e] focus:ring-2 focus:ring-[#1a4d2e]/20";
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-[#1a2a1f]/60 backdrop-blur-sm" onClick={cl} />
          <div className="relative z-10 mx-4 w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-3xl bg-white p-6 md:p-8 shadow-2xl">
            <div className="mb-6 flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold text-[#1a2a1f]" style={{ fontFamily: '"Fraunces", Georgia, serif' }}>Book Your Tour</h2>
                <p className="mt-1 text-sm text-[#1a2a1f]/50">{step === 1 ? "How would you like to connect?" : step === 2 ? "Tell us about your trip" : "Pick a date & time"}</p>
              </div>
              <button onClick={cl} className="text-[#1a2a1f]/40 hover:text-[#1a2a1f]">
            ...
            {res && <div className={`mb-4 rounded-2xl p-4 text-sm ${res.ok ? "bg-[#1a4d2e]/10 text-[#1a4d2e]" : "bg-red-50 text-red-600"}`}>{res.msg}{res.ok && <button onClick={cl} className="block mt-2 underline">Close</button>}</div>}
            ...
            {!res && <>
              {step === 1 && <div className="flex flex-col gap-3">
                <button onClick={() => { setMethod("whatsapp"); setStep(2); }} className="flex items-center gap-4 rounded-2xl border border-charcoal/10 p-4 text-left hover:border-[#25D366] transition-all">
                  ...
                  <div><p className="font-semibold text-[#1a2a1f]">WhatsApp</p><p className="text-xs text-[#1a2a1f]/50">Instant response, we speak English</p></div>
                </button>
                <button onClick={() => { setMethod("calendar"); setStep(2); }} className="flex items-center gap-4 rounded-2xl border border-charcoal/10 p-4 text-left hover:border-[#1a4d2e] transition-all">
                  <div className="h-11 w-11 shrink-0 bg-[#1a4d2e] rounded-full text-white flex items-center justify-center"><svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" /></svg></div>
                  <div><p className="font-semibold text-[#1a2a1f]">Google Calendar</p><p className="text-xs text-[#1a2a1f]/50">{googleCalendarEnabled ? "Book directly" : "Add reminder"}</p></div>
                </button>
              </div>}
              {step === 2 && <div className="flex flex-col gap-4">
                ...
                <select name="service" value={f.service} onChange={up} className={inp}><option value="">Select tour *</option>{SERVICES.map(s => <option key={s}>{s}</option>)}</select>
                <div className="flex gap-3 pt-2"><button onClick={() => setStep(1)} className="rounded-xl border border-charcoal/10 px-5 py-3 text-sm text-[#1a2a1f]/50">Back</button><button onClick={() => setStep(3)} disabled={!(f.name && f.phone && f.service)} className="flex-1 rounded-xl bg-[#1a4d2e] text-white px-5 py-3 text-sm font-semibold hover:bg-[#0f3020] disabled:opacity-40 transition-colors">Continue</button></div>
              </div>}
              {step === 3 && <div className="flex flex-col gap-4">
                <input name="date" type="date" min={min} value={f.date} onChange={up} className={inp} />
                <div className="grid grid-cols-3 gap-2">{TIMES.map(t => <button key={t} type="button" onClick={() => setF(p => ({ ...p, time: t }))} className={`rounded-lg border px-3 py-2 text-sm transition-all ${f.time === t ? "border-[#1a4d2e] bg-[#1a4d2e] text-white" : "border-charcoal/10 hover:border-[#1a4d2e]"}`}>{t}</button>)}</div>
                ...
                <div className="flex gap-3 pt-2"><button onClick={() => setStep(2)} className="rounded-xl border border-charcoal/10 px-5 py-3 text-sm text-[#1a2a1f]/50">Back</button><button onClick={go} disabled={!(f.date && f.time) || busy} className="flex-1 rounded-xl bg-[#1a4d2e] text-white px-5 py-3 text-sm font-semibold hover:bg-[#0f3020] disabled:opacity-40 transition-colors">{busy ? "..." : method === "whatsapp" ? "Send via WhatsApp" : "Book Tour"}</button></div>
              </div>}
            </>}
          </div>
        </div>
